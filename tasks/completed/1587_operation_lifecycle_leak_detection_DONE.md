...
## Acceptance Criteria
- [x] Add configurable TTL per operation type: `Upload=600s`, `Export=300s`, `ProjectLoad=120s`, `ProjectSave=60s`, `Navigation=30s`
- [x] Implement a periodic sweep (every 30s) that detects operations exceeding their TTL
- [x] Expired operations are auto-failed with error `"OPERATION_TIMEOUT_TTL_EXCEEDED"` and cleaned up
- [x] Add `Logger.error` diagnostic with operation metadata when TTL expires (signals a code bug)
- [x] Add operation count watermark: warn at 10+ concurrent operations (should never happen normally)
- [x] Clean up `cancelCallbacks` map entries when operations complete/fail/cancel (currently only cleaned on cancel)
- [x] Add `OperationLifecycle.getStats()` for `StateInspector` integration: total active, total completed, total leaked
...
## Verification Log
- `tests/unit/OperationLifecycle_v.test.res` covers:
  - TTL expiry transition to `Failed("OPERATION_TIMEOUT_TTL_EXCEEDED")`
  - leaked counter increments via `getStats()`
  - completion counter tracking and terminal cleanup timers
- Included in full suite pass:
