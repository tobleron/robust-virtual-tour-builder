...
## Acceptance Criteria
- [x] Implement 3-tier priority: `Critical` (navigation, scene load), `Normal` (upload, save/export), `Background` (thumbnail enhancement, telemetry, geocoding)
- [x] Critical requests can preempt: if all 6 slots are full with Normal/Background work, a Critical request bumps the concurrency limit temporarily (+2 burst slots)
- [x] Add starvation prevention: Background tasks get promoted to Normal after 30s of queuing, Normal to Critical after 60s
- [x] Add queue depth per-priority as telemetry (`Logger.debug` on depth changes)
- [x] Preserve existing `pause()/resume()` and `handleRateLimit()` semantics
- [x] Preserve existing `scheduleWithRetry` API with optional `~priority` parameter (default: Normal)
