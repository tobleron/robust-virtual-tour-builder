...
- [ ] Add a `Logger.warn` if total subscriptions exceed 50 (leak sentinel)
  - Implemented with `Console.warn` sentinel to avoid `EventBus` ↔ `Logger` import cycle; can be upgraded via telemetry bridge event if strict `Logger.warn` is required.
