...
## Tasks
- [ ] Implement stale task cleanup in `_dev-system/analyzer/src/main.rs`.
- [ ] Conduct a deep audit of all modules currently in `pending/` and verify they are valid concerns.
- [ ] Refine `MAP.md` sync logic to ensure all new modules are correctly classified.
- [ ] Verify build and test stability after analyzer logic updates.
