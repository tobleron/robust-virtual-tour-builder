# Task D025: Merge Folders BACKEND
...
Baseline artifacts: `_dev-system/tmp/D025/verification.json` (files at `_dev-system/tmp/D025/files/`).
Run `cargo run --manifest-path _dev-system/analyzer/Cargo.toml --bin spec_diff -- --baseline _dev-system/tmp/D025/verification.json --targets <refactored files>` once the refactor is ready to ensure the function surface matches the captured snapshots.
