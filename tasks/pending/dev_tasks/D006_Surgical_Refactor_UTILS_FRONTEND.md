...
- [ ] - **../../src/utils/PersistenceLayer.res** (Metric: [Nesting: 3.00, Density: 0.08, Coupling: 0.07] | Drag: 4.25 | LOC: 430/300  🎯 Target: Function: `normalizeProjectData` (High Local Complexity (2.0). Logic heavy.)) → 🏗️ Split into 2 modules (target ~300 LOC each)
...
- `src/utils/PersistenceLayer.res` (31 functions, fingerprint 6cd6875e8e2ae2b1f0187baa38693e6352561060a0a57f8ac4e5b601522a6ca0)
    - Grouped summary:
        - _ × 1 (lines: 248)
        - beforeUnloadListener × 1 (lines: 47)
        - checkRecovery × 1 (lines: 358)
        - clearSession × 1 (lines: 302)
        - coalesceMs × 1 (lines: 36)
        - currentSchemaVersion × 1 (lines: 35)
        - debounceMs × 1 (lines: 34)
        - decodeManifest × 1 (lines: 311)
        - decodeMetadataSlice × 1 (lines: 330)
        - encodeMetadataSlice × 1 (lines: 56)
        - extractFromSlice × 1 (lines: 324)
        - handleStateChange × 1 (lines: 250)
        - initSubscriber × 1 (lines: 264)
        - key × 1 (lines: 31)
        - lastQueuedAtMs × 1 (lines: 49)
        - lastSaveTimeout × 1 (lines: 45)
        - lastSavedRevision × 1 (lines: 46)
        - lastSliceSignatureRef × 1 (lines: 51)
        - manifestKey × 1 (lines: 32)
        - normalizeProjectData × 1 (lines: 122)
        - notifyStateChange × 1 (lines: 262)
        - pendingStateRef × 1 (lines: 50)
        - performSave × 1 (lines: 136)
        - performSaveRef × 1 (lines: 52)
        - processQueuedSave × 1 (lines: 98)
        - queueIncrementalSave × 1 (lines: 87)
        - signatureOfJson × 1 (lines: 85)
        - sliceKey × 1 (lines: 54)
        - sliceKeyPrefix × 1 (lines: 33)
        - stateGetterRef × 1 (lines: 44)
        - subscriberRef × 1 (lines: 48)
    - Detailed entries are preserved in baseline JSON (`verification.json`) for machine-level diffs.
