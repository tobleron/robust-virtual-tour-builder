...
describe("ResizerLogic", () => {
  ...
  describe("createResultFiles: Renaming Logic", () => {
    ...
    testAsync("Uses suggested name if provided", async t => {
      let meta = makeMetadata(Some("MyPhoto"))
      let result = await ResizerLogic.createResultFiles(Ok((mockBlob, meta, None)), "Original.jpg")
    ...
    testAsync("Uses PureShot format if detected (IMG_YYYYMMDD_HHMMSS_XX_SSS)", async t => {
    ...
    testAsync("Uses Legacy format if detected (_HHMMSS_XX_SSS)", async t => {
      let meta = makeMetadata(None)
    ...
    testAsync("Falls back to original base name if no format matches", async t => {
      let meta = makeMetadata(None)
    ...
    testAsync("Handles tiny file if present", async t => {
      let meta = makeMetadata(Some("Photo"))
      let tinyBlob = %raw(`new Blob(["tiny"], {type: "image/webp"})`)
      let result = await ResizerLogic.createResultFiles(Ok((mockBlob, meta, Some(tinyBlob))), "Original.jpg")
  ...
  describe("processZipResponse", () => {
    testAsync("Extracts preview, metadata, and tiny from zip", async t => {
      // Mock JSZip behavior
      let _ = %raw(`(function(){
    ...
    testAsync("Fails if essential files missing", async t => {
