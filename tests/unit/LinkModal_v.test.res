...
describe("LinkModal", () => {
  ...
  test("Retarget modal save should not dispatch manual sequence reorder updates", t => {
    let h1: hotspot = {
      linkId: "A01",
      yaw: 0.0,
      pitch: 0.0,
    ...
    let initialState = TestUtils.createMockState(
    ...
    | Some(config) =>
      ...
      | Some(saveBtn) =>
        if saveBtn.label == "Save Link" {
          ...
          let hasReorderBatch = dispatchedActions.contents->Belt.Array.some(
            action =>
              switch action {
              | Batch(actions) =>
                actions->Belt.Array.some(
                  inner =>
                    switch inner {
