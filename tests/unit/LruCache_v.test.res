...
describe("LruCache", () => {
  test("evicts least recently used entry when max is exceeded", t => {
    let evictedKeys = ref([])
    let cache = LruCache.make(~maxEntries=2, ~onEvict=((key, _) => evictedKeys := [...evictedKeys.contents, key]))
  ...
  test("shrinkTo removes oldest entries first", t => {
    let evictedKeys = ref([])
    let cache = LruCache.make(~maxEntries=5, ~onEvict=((key, _) => evictedKeys := [...evictedKeys.contents, key]))
