...
describe("ReactHotspotLayer", () => {
  test(
    "deriveDuplicateStackPlacements groups same-destination hotspots under the first anchor",
    t => {
  ...
  test("resolveStackedCoords preserves the active hotspot while it is being moved", t => {
    ...
    t
    ->expect(ReactHotspotLayer.resolveStackedCoords(movingDuplicate, placements, hotspotByLinkId))
    ->Expect.toEqual(Some({x: 300.0, y: 400.0}))
  })

  test(
    "duplicate labels stay hidden for non-anchor hotspots and hover restore delay stays explicit",
    t => {
      let anchor = makeProjectedHotspot(
        ~linkId="h1",
        ~targetSceneId=Some("scene-b"),
