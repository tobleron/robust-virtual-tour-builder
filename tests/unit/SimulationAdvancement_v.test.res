...
describe("SimulationAdvancement", () => {
  ...
  describe("Scene Completion Signal Matching", () => {
    test(
      "should advance when completion signal matches current scene",
      t => {
        let ctx = makeContext(~currentSceneId=Some("s1"), ~completedSceneId=Some("s1"), ())
        t->expect(evaluate(ctx))->Expect.toEqual(Advance)
      },
    )

    test(
      "should NOT advance when completion signal does not match current scene",
      t => {
        let ctx = makeContext(~currentSceneId=Some("s2"), ~completedSceneId=Some("s1"), ()) // Stale signal
    ...
    test(
      "should NOT advance when no completion signal is present (unless first scene)",
      t => {
        let ctx = makeContext(~currentSceneId=Some("s2"), ~completedSceneId=None, ())
        t
        ->expect(evaluate(ctx))
        ->Expect.toEqual(
          Retry({
  ...
  describe("First Scene Behavior", () => {
    test(
      "should advance immediately on first scene without waiting for completion signal",
      t => {
        let ctx = makeContext(
          ~isFirstScene=true,
          ~currentSceneId=Some("s1"),
    ...
    test(
      "should advance on first scene even with stale signal (though unlikely)",
      t => {
        let ctx = makeContext(
          ~isFirstScene=true,
          ~currentSceneId=Some("s1"),
  ...
  describe("Transition Stabilization / Busy States", () => {
    test(
      "should WAIT if navigation state is not idle",
      t => {
        let ctx = makeContext(
    ...
    test(
      "should WAIT if operation lifecycle is busy",
      t => {
        let ctx = makeContext(
  ...
  describe("Retry and Backoff Logic", () => {
    test(
      "should increment retry count and calculate backoff",
      t => {
        let ctx = makeContext(
          ~retryCount=1,
          ~maxRetries=3,
    ...
    test(
      "should STOP when max retries exceeded",
      t => {
        let ctx = makeContext(
        ...
        t
        ->expect(evaluate(ctx))
        ->Expect.toEqual(Stop({reason: "max_retries_exceeded_waiting_for_signal"}))
      },
    )

    test(
      "should retry up to max count inclusive",
      t => {
        let ctx = makeContext(
          ~retryCount=3,
          ~maxRetries=3,
