...
test.describe('Application Robustness', () => {
  ...
  test.describe('Persistence', () => {
    test('Rapid Saving during Interaction', async ({ page }) => {
      ...
      await expect(page.locator('.sidebar-branding-header')).toBeVisible();
      await expect(page.locator('text=/Something went wrong/i')).not.toBeVisible();
  ...
  test.describe('Network Resilience', () => {
    ...
    test('Optimistic Rollback on API Failure', async ({ page }) => {
      ...
      const initialCount = await page.locator('.scene-item').count();
      ...
      const finalCount = await page.locator('.scene-item').count();
