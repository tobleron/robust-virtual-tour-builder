...
pub fn flush_plans(
    ...
    for (driver_name, units) in buffer {
        ...
        let ambiguities: Vec<&WorkUnit> = units
            .iter()
        ...
        if !ambiguities.is_empty() {
            file.write_all(
                format!(
                    "## ⚠️ PRECURSOR: AMBIGUITY RESOLUTION ({})\n",
                    ambiguities.len()
                )
                .as_bytes(),
            )?;
        ...
        let structural: Vec<&WorkUnit> = units
            .iter()
            .filter(|u| matches!(u, WorkUnit::Structural { .. }))
            .collect();
        if !structural.is_empty() {
            file.write_all(
                format!("## 🏗️ STRUCTURAL REFACTOR TASKS ({})\n", structural.len()).as_bytes(),
            )?;
            for unit in structural {
                if let WorkUnit::Structural {
                    file: f,
                    action,
                    reason,
                    ..
