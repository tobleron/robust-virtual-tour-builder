use super::{strip_code_modular, CommonMetrics};
...
impl<'a> RescriptParser<'a> {
        ...
        for scope in &self.completed_scopes {
            if scope.is_function {
                ...
                if scope.complexity > max_score {
                    ...
                    self.metrics.hotspot_lines = Some((
                        scope.start_line,
                        self.metrics.loc.min(scope.start_line + 20),
                    ));
                    self.metrics.hotspot_reason = Some(format!(
                        "High Local Complexity ({:.1}). Logic heavy.",
                        scope.complexity
                    ));
                }
            }
    ...
    fn current_scope_mut(&mut self) -> &mut Scope {
        self.scope_stack
            .last_mut()
            .expect("Scope stack should never be empty (guarded root)")
    }
            ...
            if trim.starts_with("open ") || trim.starts_with("include ") {
                ...
                if parts.len() > 1 {
                    for i in 0..parts.len() - 1 {
                        ...
                        let potential_module = potential_module_part
                                ...
                                .is_uppercase()
                        {
                            if !self
                                .metrics
                                .dependencies
                                .contains(&potential_module.to_string())
                            {
