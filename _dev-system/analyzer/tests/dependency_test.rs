...
fn test_rescript_dotted_module_resolution() {
    ...
    registry.insert(
        "ComplexSystem".to_string(),
        vec!["src/systems/ComplexSystem.res".to_string()],
    );
    ...
    assert!(
        results.contains(&"src/systems/ComplexSystem.res".to_string()),
        "Failed to resolve ComplexSystem.SubModule"
    );
}
...
fn test_rust_inline_module_resolution() {
    ...
    registry.insert(
        "feature".to_string(),
        vec!["backend/src/api/feature.rs".to_string()],
    );
    ...
    assert!(
        results.contains(&"backend/src/api/feature.rs".to_string()),
        "Failed to resolve feature::Logic"
    );
}
