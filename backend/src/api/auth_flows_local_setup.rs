...
use super::{
    AuthSuccessResponse, DEVICE_COOKIE_NAME, LocalResetPayload, LocalResetResponse,
    LocalSetupBootstrapPayload, LocalSetupStatusResponse, STEP_UP_SESSION_HOURS_DEFAULT,
};
...
pub(super) async fn bootstrap_local_owner(
    ...
    let step_up_hours = super::config_i64("STEP_UP_SESSION_HOURS", STEP_UP_SESSION_HOURS_DEFAULT);
    ...
    let auth_cookie = super::create_auth_cookie(&token);
    ...
    super::log_login_attempt(
        ...
        &user.email,
        &context,
        Some(&super::hash_token(&current_device_token)),
        true,
...
pub(super) async fn reset_local_owner(
    ...
    for user in users {
        let password_hash = super::hash_password(&format!("reset-{}-{}", user.id, Uuid::new_v4()))?;
        sqlx::query(
            r#"
            UPDATE users
