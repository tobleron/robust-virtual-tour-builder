# Sponsors & Supporters

**Support the future of Robust Virtual Tour Builder**

---

## About This Project

Robust Virtual Tour Builder is an **open source** virtual tour creation platform built with ReScript and Rust. It's **free for individual brokers, developers, and personal use** under the AGPL v3 license.

**Commercial licenses** are available for brokerages and organizations that want to use this software without AGPL requirements.

[View License](LICENSE) | [Commercial Licensing](docs/security/licensing.md)

---

## Why Sponsor?

Your sponsorship directly supports:

- ✅ **Ongoing Development** - New features, bug fixes, and improvements
- ✅ **Community Support** - Responding to issues, PRs, and questions
- ✅ **Documentation** - Tutorials, guides, and examples
- ✅ **Infrastructure** - Hosting costs, build servers, testing
- ✅ **Long-term Sustainability** - Ensuring this project remains free for individuals

**100% of sponsorship funds go toward development and maintenance.**

---

## Sponsorship Tiers

### ☕ Buy Me a Coffee
**$5-10 one-time**

Perfect for individual brokers who want to say thanks.

**Benefits:**
- My eternal gratitude
- Name listed in SPONSORS.md (optional)
- Good karma for helping keep this project free

**Setup:** To enable donations, create accounts at:
- [GitHub Sponsors](https://github.com/sponsors) (recommended for open source)
- [PayPal](https://www.paypal.com/buttons) (for one-time donations)
- [Stripe](https://stripe.com/payment-links) (for credit card payments)

Once set up, replace these links in SPONSORS.md with your actual donation URLs.

---

### 🥇 Supporter
**$25/month or $250/year**

For individuals who regularly use this software and want to ensure its continued development.

**Benefits:**
- Everything in Buy Me a Coffee
- Name + logo in SPONSORS.md
- Priority support (email, 48-hour response)
- Early access to beta features
- Vote on feature priorities

*[Link will be added once GitHub Sponsors/Patreon is set up]*

---

### 🏆 Patron
**$100/month or $1,000/year**

For brokerages and businesses that benefit from this software and want to give back to the community.

**Benefits:**
- Everything in Supporter
- Large logo + featured placement in SPONSORS.md
- Direct line to developer (email/Slack)
- Custom feature requests (quoted separately)
- Public recognition on website
- Social media shout-outs

*[Contact arto.eg@gmail.com | +20 100 568 4094 for Patron sponsorship]*

---

### 🏢 Corporate Sponsor
**Custom pricing (starting at $5,000/year)**

For organizations that want to make a significant impact on the project's future.

**Benefits:**
- Everything in Patron
- Premier logo placement (top of SPONSORS.md)
- Input on roadmap priorities
- Dedicated support channel
- Custom development availability
- Speaking opportunities at user meetups
- Press release announcement

*[Contact arto.eg@gmail.com | +20 100 568 4094 for corporate sponsorship]*

---

## Current Sponsors

### 🏢 Corporate Sponsors

| Sponsor | Since | Level |
|---|---|---|
| *[Your first corporate sponsor will be here]* | - | Premier |

### 🏆 Patrons

| Sponsor | Since |
|---|---|
| *[Your first patron will be here]* | - |

### 🥇 Supporters

| Name | Since |
|---|---|
| *[Your first supporter will be here]* | - |

### ☕ Coffee Buyers

Thank you to all our coffee buyers! Your contributions add up and make a real difference.

**Recent Supporters:**
- *[List of recent one-time donors will be here]*

---

## How Funds Are Used

### Development Time (70%)
- Core feature development
- Bug fixes and maintenance
- Code review and testing
- Documentation updates

### Infrastructure (20%)
- Build servers
- Testing infrastructure
- CDN and hosting costs
- Domain and SSL certificates

### Community (10%)
- Meetup sponsorships
- Conference travel (when applicable)
- Swag for contributors
- Marketing and outreach

### Financial Transparency

**Monthly Reports:**
- Income from sponsorships
- Income from commercial licenses
- Expenses (hosting, tools, etc.)
- Development hours invested

Reports are published monthly in the project's GitHub Discussions.

---

## Alternative Ways to Support

Can't sponsor financially? You can still help!

### 1. ⭐ Star This Project
Takes 2 seconds and helps with visibility.

[Star on GitHub]

### 2. 📢 Spread the Word
- Share on social media
- Tell other brokers about it
- Write a blog post
- Speak at meetups

### 3. 🐛 Report Bugs
Found a bug? Report it!

[Open an Issue]

### 4. 💡 Suggest Features
Have an idea? Share it!

[Start a Discussion]

### 5. 📖 Improve Documentation
See a typo? Confusing section? Fix it!

[Submit a PR]

### 6. 🤝 Help Others
Answer questions in discussions or issues.

[Join Discussions]

### 7. 📝 Write Tutorials
Create content showing how to use the software.

### 8. 🌍 Translate
Help translate the software to other languages.

---

## Frequently Asked Questions

**Q: Is my sponsorship tax-deductible?**  
A: Currently, no. We're exploring non-profit sponsorship options for the future.

**Q: Can I cancel my sponsorship?**  
A: Yes, you can cancel recurring sponsorships at any time.

**Q: Do you offer refunds?**  
A: Sponsorships are donations and are non-refundable. However, if you're unsatisfied, just let us know and we'll work it out.

**Q: Can I sponsor anonymously?**  
A: Yes, you can choose to remain anonymous. Just let us know when you sponsor.

**Q: Can I change my sponsorship amount?**  
A: Yes, you can increase, decrease, or cancel at any time.

**Q: Do you accept cryptocurrency?**  
A: Yes! Contact us for crypto donation addresses.

**Q: I represent a non-profit. Do you offer discounts?**  
A: Yes! Non-profits receive 50% off commercial licenses. Contact us.

**Q: What if I want to sponsor but my company needs a commercial license anyway?**  
A: Great question! Your commercial license fee already supports development. Additional sponsorship is appreciated but not required.

---

## Corporate Partnership Opportunities

Beyond sponsorship, we offer partnership opportunities for organizations aligned with our mission:

### Technology Partners
- Integration partnerships
- Co-marketing opportunities
- Joint webinars and content

### Industry Partners
- Real estate associations
- Photography organizations
- Developer communities

[Contact for Partnership Opportunities]

---

## Contact

**Questions about sponsorship?**

- Email: arto.eg@gmail.com | +20 100 568 4094
- Website: https://www.robust-vtb.com/sponsor (coming soon)

**Ready to sponsor?**

Choose your tier above and click the donation link, or contact arto.eg@gmail.com | +20 100 568 4094 for custom arrangements.

---

## Thank You

To all our sponsors, supporters, and contributors: **thank you**.

Your support makes it possible to keep this software free for individual brokers, developers, and hobbyists while continuing to add new features and improvements.

**Together, we're building the future of virtual tour creation.**

---

**Last Updated:** March 19, 2026

*[This page is automatically updated when new sponsors are added]*
