...
let resetState = () => {
  ...
  [
    state.contents.viewportSaveTimeout,
    state.contents.idleSnapshotTimeout,
    state.contents.loadSafetyTimeout,
  ]
  ->Belt.Array.keepMap(Nullable.toOption)
  ->Belt.Array.forEach(Window.clearTimeout)

  state := {
    lastMouseEvent: Nullable.null,
    guide: state.contents.guide, // Preserve guide if it's already found in DOM
    lastPreloadingIndex: -1,
    mouseXNorm: 0.0,
    mouseYNorm: 0.0,
    followLoopActive: false,
