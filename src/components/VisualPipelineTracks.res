...
let nodeShiftStyle = (
  ...
  let (sx, sy) = switch stableHubTargetClusters->Belt.MutableMap.String.get(node.representedSceneId) {
  | Some(cluster) =>
    ...
    ->Belt.Array.map(fid => {
      ...
      >
        ...
        ->Belt.Array.mapWithIndex((idx, node) => {
          ...
          let representedScene = pipelineScenes->Belt.Array.getBy(s => s.id == node.representedSceneId)
