...
let make = () => {
  ...
  <>
    ...
    // Logic for disabling buttons
    {
      ...
      let viewerBarClass =
        "absolute top-6 left-5 z-[5002] flex flex-col gap-3 items-start bg-transparent" ++ if (
          !scenesLoaded
        ) {
      ...
      <div id="viewer-utility-bar" className={viewerBarClass}>
        <button
          id="btn-add-link-fab"
          className={"w-[32px] h-[32px] text-white rounded-full font-ui text-[20px] font-bold flex items-center justify-center btn-viewer-pop leading-none pb-0.5 relative z-[6000] " ++ if (
            scenesLoaded
          ) {
            "pointer-events-auto"
          } else {
        ...
        <button
          ...
          className={"w-[32px] h-[32px] text-white rounded-full font-ui flex items-center justify-center btn-viewer-pop relative z-[6000] " ++ if (
            scenesLoaded
          ) {
            "pointer-events-auto"
          } else {
          ...
          title={if simActive {
            "Stop Auto-Pilot Simulation"
          } else {
            "Start Auto-Pilot Simulation"
          }}
        >
          <span
            className="material-icons" style={makeStyle({"fontSize": "19px", "color": "white"})}
          >
            {React.string(
              if simActive {
                "stop"
        ...
        <button
          ...
          className={"w-[32px] h-[32px] text-white rounded-full flex items-center justify-center btn-viewer-pop relative z-[6000] " ++ if (
            scenesLoaded
          ) {
            "pointer-events-auto"
          } else {
        ...
        <button
          ...
          className={"w-[32px] h-[32px] text-white rounded-full font-ui text-[18px] font-bold flex items-center justify-center btn-viewer-pop relative z-[6000] " ++ if (
            scenesLoaded
          ) {
            "pointer-events-auto"
          } else {
    ...
    <div
      ...
      className="hidden absolute top-6 left-1/2 -translate-x-1/2 z-[6005] text-white px-3 py-0.5 rounded-md text-[12px] font-black uppercase shadow-lg items-center justify-center transition-all duration-300 pointer-events-none border border-white/20 opacity-0 -translate-y-2 scale-95 tracking-wider"
      style={makeStyle({"backgroundColor": "#2563eb"})}
    >
    </div>

    <div
      id="v-scene-quality-indicator"
      className="hidden absolute top-6 right-6 z-[6005] flex items-center gap-2 pointer-events-none transition-all duration-300 opacity-0 translate-x-2 scale-95"
    >
    </div>
...

    {
      let scenesLoaded = Belt.Array.length(state.scenes) > 0
      let floorNavClass =
        "absolute bottom-6 left-5 z-[5002] flex flex-col-reverse gap-2 items-center" ++ if (
          !scenesLoaded
        ) {
          " opacity-50 pointer-events-none grayscale"
        } else {
          ...
          >
            {if f.suffix != "" {
              <>
                ...
                <sup style={makeStyle({"fontSize": "10px", "marginLeft": "1px"})}>
