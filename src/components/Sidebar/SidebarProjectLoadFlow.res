...
let finalizeLoadedProject = async (
  ~sessionId: string,
  ~projectData: JSON.t,
...
let handleSavedProjectLoad = async (
  ~sessionId: string,
  ~projectData: JSON.t,
  ~label: string,
  ~getState: unit => Types.state,
  ...
  dispatch(Actions.DispatchAppFsmEvent(StartProjectLoad({name: label})))
  ...
  Logger.startOperation(
    ...
    ~data={"label": label, "source": "saved_project"},
    (),
  )
  ...
  try {
    if OperationLifecycle.isActive(opId) {
      OperationLifecycle.progress(
        opId,
        42.0,
        ~message="Preparing saved project...",
        ~phase="Project Load",
        (),
      )
    }
  ...
  | _ =>
    let msg = "Unexpected project load error"
    SidebarProjectLoadLifecycle.finalizeFailure(~dispatch, ~opId, ~loadSettled, ~msg)
    JsError.throwWithMessage(msg)
  }
}
