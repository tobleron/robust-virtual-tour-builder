...
let localAssetCount = (state: Types.state): int => {
  SidebarSaveSupport.localAssetCount(state)
}

let saveToServer = async (~state, ~dispatch, ~onPhase: option<string => unit>=?) => {
  await SidebarSaveSupport.saveToServer(~state, ~dispatch, ~onPhase?)
}

let handleSave = async (~mode, ~getState, ~signal, ~onCancel, ~dispatch) => {
  await SidebarSaveSupport.handleSave(~mode, ~getState, ~signal, ~onCancel, ~dispatch)
}
