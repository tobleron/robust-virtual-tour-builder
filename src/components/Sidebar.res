...
module Exporter = {
  @module("../systems/Exporter.js")
  external exportTour: (array<Store.scene>, (float, float, string) => unit) => promise<unit> = "exportTour"
}

module ProjectManager = {
  @module("../systems/ProjectManager.js")
  external saveProject: (Store.state, (float, float, string) => unit) => promise<unit> = "saveProject"
  @module("../systems/ProjectManager.js")
  external loadProject: ('file, (float, float, string, bool) => unit) => promise<JSON.t> = "loadProject"
}
...
module UploadProcessor = {
    // result type optional
    @module("../systems/UploadProcessor.js") @scope("UploadProcessor")
    external processUploads: (array<Dom.element>, (float, string, bool, string) => unit) => promise<JSON.t> = "processUploads"
}

module UploadReport = {
    @module("./UploadReport.js") @scope("UploadReport")
    external show: (Store.uploadReport, JSON.t) => unit = "show"
}
...
let make = () => {
    ...
    // Store subscription
    React.useEffect0(() => {
        Store.store.subscribe(newState => setState(_ => newState))
        None
    })
    ...
    let handleUpload = async (e) => {
        ...
        if files["length"] > 0 {
           ...
           // let fileArray = Js.Array.from(files) // if exists
           let fileArray = (Obj.magic(files): array<Dom.element>) // Assuming array-like works or strict array needed?
           // JS `Array.from(files)` is safer.
           
           try {
               let result = await UploadProcessor.processUploads(fileArray, (pct, msg, isProc, phase) => {
                   updateProgress(pct, msg, isProc, phase)
               })
               
               let resObj = (Obj.magic(result): {"qualityResults": JSON.t})
               UploadReport.show(state.lastUploadReport, resObj["qualityResults"]) 
           } catch {
           | JsExn(obj) => 
               let msg = switch JsExn.message(obj) { | Some(m) => m | None => "Unknown error" }
    ...
    // Render
    <div className="relative w-[320px] min-w-[320px] bg-white flex flex-col z-[15000] shrink-0 h-full overflow-hidden font-ui shadow-2xl">
       /* Branding Header */
       <div className="relative w-full flex flex-col z-30 text-white shrink-0" 
            style={makeStyle({"borderTop": "2px solid #dc3545", "background": "linear-gradient(to bottom, #001a38 0%, #002a70 50%, #003da5 100%)"})}>
            
            <div className="flex flex-col items-center px-5 pb-4" style={makeStyle({"paddingTop": "23px"})}>
                <span className="material-icons text-white drop-shadow-lg mb-0.5 mt-1" style={makeStyle({"fontSize": "36px"})}>{React.string("home")}</span>
                <h1 className="font-black text-white tracking-tight drop-shadow-sm text-center" style={makeStyle({"fontSize": "24px"})}>{React.string("Virtual Tour Builder")}</h1>
            ...
            <div style={makeStyle({"padding": "0 16px 14px 16px"})}>
                ...
                <div className="sidebar-btn-grid-4">
                    <button className="sidebar-action-btn-square" title="New Project" 
                        onClick={_ => {
                            if Array.length(state.scenes) > 0 {
                                ModalManager.show({
                                    title: "Create New Project?",
                                    description: Some("Are you sure you want to discard the current project? All unsaved progress will be lost."),
                                    icon: Some("warning"),
                                    contentHtml: None,
                                    buttons: [
                                        {label: "Cancel", class: "bg-slate-100 text-slate-700 hover:bg-slate-200", onClick: None, autoClose: Some(true)},
                                        {label: "Discard & New", class: "bg-red-500 text-white hover:bg-red-600", onClick: Some(() => reload()), autoClose: Some(true)}
                                    ]
                                })
                            } else {
                                reload()
                    ...
                    <button className="sidebar-action-btn-square" title="Save Project" 
                        onClick={_ => {
                            let _ = (async () => {
                    ...
                    <button className="sidebar-action-btn-square" title="Load Project" onClick={_ => {
                    ...
                    <button className="sidebar-action-btn-square group" title="About" onClick={_ => {
                        ModalManager.show({
                            ...
                            contentHtml: None,
                            buttons: [
                                {label: "Close", class: "bg-slate-100 text-slate-700", onClick: None, autoClose: Some(true)}
                            ]
                        })
                    }}>
                        <span className="material-icons group-hover:scale-110 transition-transform">{React.string("info")}</span>
                ...
                <div className="sidebar-btn-grid-2">
                    <button className="sidebar-action-btn-wide"
                            disabled={!exportReady}
                            title="Export Tour"
                            style={if !exportReady {makeStyle({"opacity": "0.4"})} else {makeStyle(Object.make())}}
                            onClick={_ => {
                                let _ = (async () => {
                                     updateProgress(0.0, "Initializing Export...", true, "Exporting")
                                     try {
                                         let _ = await Exporter.exportTour(state.scenes, (pct, _total, msg) => updateProgress(pct, msg, true, "Exporting"))
                        ...
                        <span className="material-icons" style={makeStyle({"color": "#10b981"})}>{React.string("ios_share")}</span>
                        <span>{React.string("Export")}</span>
                    </button>
                    
                     <button className="sidebar-action-btn-wide"
                            disabled={!teaserReady}
                            title={if teaserReady {"Auto-generate teaser video"} else {"Need at least 3 scenes"}}
                            style={if !teaserReady {makeStyle({"opacity": "0.4"})} else {makeStyle(Object.make())}}
                            onClick={_ => {
                                TeaserSystem.startAutoTeaser(state.tourName, false, "mp4", false)
                            }}>
                        <span className="material-icons" style={makeStyle({"color": "#f97316"})}>{React.string("movie_creation")}</span>
                ...
                /* Hidden Inputs */
                <input type_="file" ref={ReactDOM.Ref.domRef(fileInputRef)} multiple=true accept="image/jpeg,image/png,image/webp" className="hidden" onChange={e => { let _ = handleUpload(e) }}/>
                <input type_="file" ref={ReactDOM.Ref.domRef(projectFileInputRef)} accept=".vt.zip,.zip" className="hidden" 
                       onChange={e => {
                            let _ = (async () => {
                                let target = JsxEvent.Form.target(e)
                                let files = target["files"]
                                if files["length"] > 0 {
                                    ...
                                    try {
                                        let file = files["0"]
                                        let projectData = await ProjectManager.loadProject(file, (pct, _t, msg, active) => {
                                            updateProgress(pct, msg, active, "Loading")
                                        })
                                        Store.store.loadProject(projectData)
                                        ReBindings.Notification.notify("Project loaded", "success")
                                        updateProgress(100.0, "Loaded", false, "")
                                    } catch {
                                    | JsExn(obj) => 
                                        let msg = switch JsExn.message(obj) { | Some(m) => m | None => "Unknown error" }
                                        ReBindings.Debug.error("Sidebar", "Load failed: " ++ msg, ())
       ...
       <div className="flex flex-col bg-slate-50 border-b border-slate-200 shadow-sm shrink-0 z-20">
            <div className="p-4 pt-5 pb-3">
                 <div className="flex items-center justify-between mb-1.5 px-1">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{React.string("Project Name")}</label>
                        <span className="text-[9px] font-bold text-remax-blue/60 uppercase">{React.string("Draft Mode")}</span>
                </div>
                <input type_="text" 
                       id="tour-name-input"
                       ...
                       onChange={e => {
                           ...
                           Store.store.setTourName(val)
                       }} />
            </div>
        
            <div className="px-4 pb-4" 
                 style={makeStyle({"display": if procState["active"] {"none"} else {"block"}})}>
                <label id="upload-label"
                    ...
                    <div className="w-6 h-6 rounded-full bg-remax-blue/10 flex items-center justify-center group-hover:bg-white/20 transition-colors">
                        <span className="material-icons text-[15px] text-remax-blue group-hover:text-white transition-colors">{React.string("cloud_upload")}</span>
                    </div>
                    <strong className="text-[11px] font-bold tracking-tight text-slate-600 group-hover:text-white">{React.string("Upload 360 Images")}</strong>
                </label>
            </div>
       </div>

       /* Sidebar Content Area - Scrollable */
       <div className="sidebar-content flex-1 overflow-y-auto overflow-x-hidden hide-scrollbar flex flex-col bg-white">
            /* Processing UI Card - Inside scrollable area */

           {if procState["active"] {
               <div className="m-4 bg-white border border-slate-100 rounded-xl p-4 shadow-xl ring-1 ring-remax-blue/5 animate-fade-in shrink-0">
                    <div className="flex items-center justify-between mb-3">
                         <div className="flex items-center gap-2">
                             <div className="w-3 h-3 border-2 border-slate-100 border-t-remax-blue rounded-full animate-spin"></div>
                             <div className="font-bold text-remax-blue text-[10px] uppercase tracking-wide">{React.string(procState["phase"] == "" ? "Processing" : procState["phase"])}</div>
                         </div>
                         <div className="font-black text-remax-blue text-xs font-heading">{React.string(Float.toString(procState["progress"]) ++ "%")}</div>
                     </div>
                    <div className="bg-slate-100 h-1.5 rounded-full overflow-hidden relative">
                        <div className="h-full bg-remax-blue transition-all duration-300 rounded-full relative"
                    ...
                    <div className="text-[9px] text-slate-400 mt-2 font-bold uppercase tracking-tighter flex items-center gap-1.5">
