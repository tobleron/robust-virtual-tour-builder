...
let findCustomerOverview = (
  ...
  customers->Belt.Array.getBy((customer: PortalTypes.customerOverview) => customer.customer.id == customerId)
...
let parseRoute = () => {
  ...
  | 5 =>
    ...
    | (Some("portal"), Some(slug), Some("tour"), Some(tourSlug)) =>
      CustomerTour(slug, tourSlug)
    | (Some("u"), Some(slug), Some("tour"), Some(tourSlug)) =>
      CustomerTour(slug, tourSlug)
    | _ => Landing
    }
  | _ => Landing
  ...
  <div className="portal-brand-stack">
    <span className="portal-brand-logo-lockup">
      <img
        ...
        alt="Robust Virtual Tour Builder logo"
      />
...
module CopyActionButton = {
  ...
  let make = (
      ...
      if !disabled {
        ignore(
          (async () => {
            ...
            | _ =>
              onCopyError->Option.forEach(report =>
                report("Unable to copy link automatically.")
              )
            }
          })(),
        )
    ...
    let icon = if isCopied { CheckIcon } else { CopyIcon }
    let resolvedLabel = if isCopied { copiedLabel } else { label }
    let resolvedTitle = if isCopied { copiedLabel } else { title }
    <button
      className={className ++ (if isCopied { " is-copied" } else { "" })}
      disabled={disabled}
      ariaLabel
      title={resolvedTitle}
      onClick={handleClick}>
...
let loadPortalTourDocument = async (~slug, ~tourSlug) => {
  ...
  let rec tryCandidate = async index => {
    ...
    | Some(entryDir) =>
      ...
      let response = await Fetch.fetch(baseHref ++ "index.html", Fetch.requestInit(~method="GET", ()))
...
let paginateArray = (items: array<'a>, page: int) => {
  let safePage = if page < 0 { 0 } else { page }
  let startIndex = safePage * pageSize
  let paged = items->Belt.Array.keepWithIndex((_, index) => index >= startIndex && index < startIndex + pageSize)
...
let matchesRecipientSearch = (overview: PortalTypes.customerOverview, query: string) => {
  ...
  if normalizedQuery == "" {
    ...
    displayName->String.includes(normalizedQuery) || slug->String.includes(normalizedQuery)
  }
}

let matchesRecipientFilter = (overview: PortalTypes.customerOverview, filter: recipientStatusFilter) =>
  switch filter {
  | RecipientAll => true
      ...
      <main className="portal-main portal-auth-main">
        <section className="portal-hero portal-auth-card">
          ...
          <p className="portal-subtitle">
            {React.string("Sign in with your internal Robust account to manage portal recipients and tours.")}
          </p>
          {messageNode(~flash)}
...
module AdminSurface = {
  ...
  let make = () => {
    ...
    let (customerDrafts, setCustomerDrafts) = React.useState(
      (): Belt.Map.String.t<customerDraft> => Belt.Map.String.empty,
    )
    let (expiryDrafts, setExpiryDrafts) = React.useState(
      (): Belt.Map.String.t<string> => Belt.Map.String.empty,
    )
    let (lastGeneratedLinks, setLastGeneratedLinks) = React.useState(
      (): Belt.Map.String.t<string> => Belt.Map.String.empty,
    )
    ...
    let loadAdmin = (~preserveReadyState=false) => {
      ...
      ignore(
        (async () => {
          ...
          | RefreshAuth =>
            setState(_ => Failed("AUTH"))
          | RefreshError(message) =>
            setState(_ => Failed(message))
          | RefreshOk(payload) =>
            applyAdminData(payload)
          }
    ...
    let updateCustomerDraft = (customerId, updater) =>
      setCustomerDrafts(prev => {
        let current = prev->Belt.Map.String.get(customerId)->Option.getOr({
          displayName: "",
          isActive: true,
        })
    ...
    let onSaveSettings = async () => {
      ...
      | None => ()
      | Some(draft) =>
        switch await PortalApi.updateSettings(~settings={
    ...
    let onCreateCustomer = async () => {
      ...
      switch await PortalApi.createCustomer(~slug, ~displayName, ~expiresAt, ~contactName=None, ~contactEmail=None, ~contactPhone=None) {
      | Ok(result) =>
        setLastGeneratedLinks(prev =>
          prev->Belt.Map.String.set(result.overview.customer.id, result.accessLink.accessUrl)
        )
        setCreateDraft(_ => {slug: "", displayName: "", expiresAt: nowPlusDaysIsoLocal(30)})
        setActiveDrawer(_ => NoDrawer)
    ...
    | Failed("AUTH") =>
      <AdminSignIn.make flash onSignIn={onAdminSignIn} />
    | Failed(message) =>
    ...
    | Ready(data) =>
      ...
      let selectedOverview =
        selectedCustomerId
        ->Option.flatMap(customerId => findCustomerOverview(data.customers, customerId))
      let filteredRecipients =
        data.customers
        ->Belt.Array.keep(customerOverview =>
          matchesRecipientSearch(customerOverview, recipientSearch) &&
            matchesRecipientFilter(customerOverview, recipientFilter)
        )
      let (visibleRecipients, recipientTotal, recipientHasPrev, recipientHasNext) =
        paginateArray(filteredRecipients, recipientPage)
      let filteredTours =
        data.tours
        ->Belt.Array.keep(tourOverview =>
            ...
            <aside className="portal-drawer" onClick={event => ReactEvent.Mouse.stopPropagation(event)}>
              ...
              <div className="portal-form-grid">
                <label>
                  ...
                  <input
                    ...
                    onChange={e =>
                      setSettingsEdit(_ =>
                        Some({...settingsDraft, renewalHeading: ReactEvent.Form.target(e)["value"]})
                      )}
                  />
                </label>
                <label>
                  ...
                  <input
                    ...
                    onChange={e =>
                      setSettingsEdit(_ =>
                        Some({...settingsDraft, contactEmail: ReactEvent.Form.target(e)["value"]})
                      )}
                  />
                </label>
                <label>
                  ...
                  <input
                    ...
                    onChange={e =>
                      setSettingsEdit(_ =>
                        Some({...settingsDraft, contactPhone: ReactEvent.Form.target(e)["value"]})
                      )}
                  />
                </label>
                <label>
                  ...
                  <input
                    ...
                    onChange={e =>
                      setSettingsEdit(_ =>
                        Some({...settingsDraft, whatsappNumber: ReactEvent.Form.target(e)["value"]})
                      )}
                  />
                </label>
              </div>
              <label className="portal-form-field">
                ...
                <textarea
                  ...
                  onChange={e =>
                    setSettingsEdit(_ =>
                      Some({...settingsDraft, renewalMessage: ReactEvent.Form.target(e)["value"]})
                    )}
                />
              </label>
              <div className="portal-form-actions">
                  ...
                  <div className="portal-form-grid">
                    ...
                    <label>
                      ...
                      <input
                        ...
                        onChange={e => setConfirmNextPassword(_ => ReactEvent.Form.target(e)["value"])}
                      />
                    </label>
                  </div>
                  <div className="portal-form-actions">
                    <button className="site-btn site-btn-primary" onClick={_ => ignore(onChangePassword())}>
            ...
            <article className="portal-card portal-admin-toolbar-card">
              <div className="portal-toolbar-head">
                <div>
                  ...
                  <p className="portal-card-muted">
                    {React.string(
                      "Open the right drawer action, then use the directory to move between recipients without losing your place.",
                    )}
                  </p>
                </div>
              </div>
            ...
            <div className="portal-admin-main">
              <article className="portal-card">
                <div className="portal-section-head">
                  <div>
                    ...
                    <p className="portal-card-muted">
                      {React.string("Scan status, expiry, and recent activity in one full-width workspace table.")}
                    </p>
                  </div>
                  <div className="portal-inline-actions">
                    <span className="portal-chip">
                      {React.string(
                        "Showing " ++ Belt.Int.toString(visibleRecipients->Belt.Array.length) ++
                        " of " ++ Belt.Int.toString(recipientTotal),
                      )}
                    </span>
                  </div>
                </div>
                <div className="portal-table-scroll">
                  <div className="portal-recipient-table">
                    ...
                    | _ =>
                      visibleRecipients->Belt.Array.map(customerOverview => {
                        let isSelected =
                          selectedCustomerId
                        ...
                        <button
                          ...
                          className={"portal-recipient-row " ++ (isSelected ? "is-selected" : "")}
                          ariaLabel={"Open recipient " ++ customerOverview.customer.displayName}
                          onClick={_ => setSelectedCustomerId(_ => Some(customerOverview.customer.id))}>
                          ...
                          <span className="portal-table-cell portal-table-cell-action">
                            ...
                            <span className="portal-table-action-chip">
                              {actionLabel(~icon=DetailIcon, ~label="Details")}
                            </span>
                          </span>
                        </button>
                      })->React.array
                ...
                <div className="portal-pagination">
                  ...
                  <span className="portal-row-muted">
                    {React.string(
                      "Page " ++ Belt.Int.toString(recipientPage + 1) ++ " of " ++
              ...
              | None =>
                <article className="portal-card portal-empty-state">
                  ...
                  <p className="portal-card-muted">
                    {React.string("Select a recipient from the directory to manage links, status, and assignments.")}
                  </p>
                </article>
              | Some(customerOverview) =>
                ...
                let activeAccessUrl = lastLink->Option.orElse(
                  customerOverview.accessLink->Option.flatMap(link => link.accessUrl)
                )
                let galleryExpiryLabel =
                  if expiryDraft == "" {
                ...
                <article className="portal-card portal-recipient-detail-card">
                  ...
                  <div className="portal-detail-grid">
                      ...
                      <div className="portal-form-grid">
                        ...
                        <label>
                          ...
                          <input
                            type_="datetime-local"
                            value={expiryDraft}
                            onChange={e => updateExpiryDraft(customerId, ReactEvent.Form.target(e)["value"])}
                          />
                        </label>
                        <label className="portal-toggle-field">
                          ...
                          <span className="portal-toggle-control">
                            <input
                              ...
                              onChange={_ =>
                                updateCustomerDraft(customerId, draft => {...draft, isActive: !draft.isActive})}
                            />
                            <strong>
                              {React.string(if customerDraft.isActive { "Recipient active" } else { "Recipient inactive" })}
                            </strong>
                          </span>
                        </label>
                      </div>
                    ...
                    <div className="portal-detail-card portal-detail-card-danger">
                      ...
                      <p className="portal-card-muted">
                        {React.string("Remove links or recipients only when cleaning invalid data or test records.")}
                      </p>
                      <div className="portal-inline-actions portal-inline-actions-compact">
                        <button
                  ...
                  <div className="portal-section-head portal-section-head-tight">
                    <div>
                      ...
                      <p className="portal-card-muted">
                        {React.string("Share the gallery with the client, or use direct broker links for assigned tours.")}
                      </p>
                    </div>
                  </div>
                      ...
                      <div className="portal-link-card">
                        <div className="portal-link-copy">
                          ...
                          <span className="portal-link-value">
                            {React.string("Copy to share the private gallery URL, or open it to verify the experience first.")}
                          </span>
                        </div>
                        <div className="portal-inline-actions portal-inline-actions-compact">
                          <CopyActionButton
                            ...
                            onCopyError={message => setFlash(_ => {error: Some(message), success: None})}
                          />
                          <button
                            className="site-btn site-btn-ghost portal-compact-btn"
                  ...
                  <div className="portal-section-head">
                    <div>
                      ...
                      <p className="portal-card-muted">
                        {React.string("Assign tours here, then use copy/open actions once a gallery link is active.")}
                      </p>
                    </div>
                  </div>
                  <div className="portal-table-scroll">
                    <div className="portal-assignment-table">
                      ...
                      {data.tours->Belt.Array.map(tourOverview => {
                        let assigned =
                        ...
                        <div key={tourOverview.tour.id} className="portal-assignment-row">
                          ...
                          <span className="portal-table-cell">
                            {mobileLabel("Status")}
                            <span className={"portal-chip " ++ (tourOverview.tour.status == "published" ? "is-published" : "")}>
                            ...
                            | Some(accessUrl) if assigned =>
                              {
                                ...
                                <div className="portal-link-summary">
                                  ...
                                  <small className="portal-row-muted">
                                    {React.string("Bypasses the gallery and opens this assigned tour immediately.")}
                                  </small>
                                  <div className="portal-inline-actions portal-inline-actions-compact">
                                    <CopyActionButton
                                      className="site-btn site-btn-ghost portal-compact-btn portal-copy-btn"
                          ...
                          <span className="portal-table-cell portal-table-cell-action">
                            ...
                            <div className="portal-table-action-cluster">
                              <button
                                className={
                                  "site-btn site-btn-ghost portal-compact-btn " ++
              ...
              <article className="portal-card">
                <div className="portal-section-head">
                  <div>
                    ...
                    <p className="portal-card-muted">
                      {React.string("Keep the tour library dense and reusable so assignments stay quick to operate.")}
                    </p>
                  </div>
                ...
                <div className="portal-table-scroll">
                  <div className="portal-library-table">
                    <div className="portal-table-head portal-library-table-head">
                      ...
                      <span> {React.string("Actions")} </span>
                    </div>
                    {visibleTours->Belt.Array.map(tourOverview => {
                        ...
                        <span className="portal-table-cell portal-table-cell-action">
                          ...
                          <div className="portal-table-action-cluster">
                            <button
                              ...
                              onClick={_ => ignore(onTourStatus(~tourId=tourOverview.tour.id, ~status="published"))}>
                            ...
                            <button
                              ...
                              disabled={isDraft}
                              onClick={_ => ignore(onTourStatus(~tourId=tourOverview.tour.id, ~status="draft"))}>
                            ...
                            <button
                              ...
                              disabled={isArchived}
                              onClick={_ => ignore(onTourStatus(~tourId=tourOverview.tour.id, ~status="archived"))}>
                            ...
                            <button
                              ...
                              onClick={_ =>
                                ignore(onDeleteTour(~tourId=tourOverview.tour.id, ~title=tourOverview.tour.title))}>
                ...
                <div className="portal-pagination">
                  ...
                  <span className="portal-row-muted">
                    {React.string(
                      "Page " ++ Belt.Int.toString(tourPage + 1) ++ " of " ++ Belt.Int.toString(tourPageCount) ++
...
module CustomerSurface = {
  ...
  let make = (~slug, ~tourSlug: option<string>) => {
    let (publicState, setPublicState) = React.useState(
      (): remoteData<PortalTypes.publicView> => Loading,
    )
    let (sessionState, setSessionState) = React.useState(
      (): remoteData<PortalTypes.customerSession> => Idle,
    )
    let (galleryState, setGalleryState) = React.useState(
      (): remoteData<PortalTypes.galleryView> => Idle,
    )
    let (tourDocumentState, setTourDocumentState) = React.useState(
      (): remoteData<string> => Idle,
    )
    ...
    React.useEffect2(() => {
      ...
      ignore(
        (async () => {
          ...
          | Error(message) =>
            setPublicState(_ => Failed(message))
          | Ok(publicView) =>
      ...
      None
    }, (slug, switch tourSlug { | Some(value) => value | None => "" }))
      ...
      <div className="portal-shell">
        <main className="portal-main portal-customer-main">
          <section className="portal-hero portal-customer-hero">
            ...
            | Some(message) => <div className="portal-message is-error"> {React.string(message)} </div>
            | None => <div className="portal-message"> {React.string("Open this portal through your private access link.")} </div>
            }}
          </section>
          ...
          | Ready(gallery) =>
            ...
            <div className="portal-shell">
              <main className="portal-main portal-customer-main">
                ...
                {!gallery.canOpenTours ? <section className="portal-card">
                ...
                <section className="portal-card portal-customer-gallery">
                  ...
                  {if totalTours == 0 {
                    ...
                    <div className="portal-grid-tours">
                      {gallery.tours->Belt.Array.map(tour => {
                        let shareUrl =
                        ...
                        <article key={tour.id} className="portal-tour-card">
                          ...
                          <div className="portal-tour-copy">
                            <div className="portal-chip-row">
                              <span className={"portal-chip " ++ (tour.status == "published" ? "is-published" : "")}>
