...
let analyzeGroups = (exifResults: array<exifResult>, lines) => {
  ...
  Belt.Array.forEach(Dict.toArray(groups), ((signature, files)) => {
    ...
    | Some(r) => r.exifData
    | None => SharedTypes.defaultExif
    }

    let dashCount = ExifUtils.maxInt(0, 60 - String.length(signature))
...
let listIndividualFiles = (exifResults: array<exifResult>, lines) => {
  Belt.Array.forEach(exifResults, r => {
